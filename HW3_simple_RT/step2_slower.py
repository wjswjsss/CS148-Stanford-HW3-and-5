...
def RT_trace_ray(scene, ray_orig, ray_dir, lights, depth=0):
    ...
    for light in lights:
        ...
        """
        Not using the half-way vec
        """
        L = (light.location - hit_loc).normalized()
        R = 2 * (L.dot(hit_norm)) * hit_norm - L
        specular = I * pow(((-ray_dir).dot(R)), specular_hardness) * specular_color
